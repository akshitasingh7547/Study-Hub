        ...
        <div className="col-span-1 bg-gray-900/60 p-6 rounded card-dark">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            {Object.entries(STUDY_MODES).map(([key, v]) => (
              <button key={key} onClick={() => { setMode(key); setIsRunning(false); setIsBreak(false); setTimeLeft(v.focus) }} className={`py-2 rounded ${mode===key ? 'bg-yellow-500 text-black' : 'bg-gray-800/30'}`}>{v.name.split(' ')[0]}</button>
            ))}
          </div>
